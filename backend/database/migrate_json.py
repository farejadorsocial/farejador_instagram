...
def migrar_usuarios(session: Session, dry_run: bool) -> int:
    ...
    for caminho in sorted(USER_ROOT.glob("*/usuario.json")):
        ...
        if not dry_run:
            session.add(
                Usuario(
                    username=str(usuario.get("username") or username).strip().lower(),
                    password_hash=password_hash,
                    salt=salt,
                    criado_em=normalizar_datetime(conta.get("criado_em")),
                    ultimo_login=normalizar_datetime(conta.get("ultimo_login")),
                    ativo=bool(conta.get("ativo", True)),
                    configuracoes=configuracoes if isinstance(configuracoes, dict) else {},
                )
            )
        inseridos += 1
...
def migrar_sessoes(session: Session, dry_run: bool) -> int:
    ...
    for caminho in sorted(iterar_json(USER_ROOT, "sessoes.json")):
        ...
        for dados in ler_lista(caminho):
            ...
            if session.scalar(select(Usuario).where(Usuario.username == cliente)) is None:
            ...
            if not dry_run:
                session.add(
                    Sessao(
                        ...
                        username=cliente,
                        criada_em=normalizar_datetime(dados.get("criada_em") or status.get("criada_em")),
                        ultimo_acesso=normalizar_datetime(dados.get("ultimo_acesso") or status.get("ultimo_acesso")),
                        expira_em=normalizar_datetime(dados.get("expira_em") or status.get("expira_em")),
                        ativa=bool(status.get("ativa", True)),
                        acesso=dados.get("acesso") if isinstance(dados.get("acesso"), dict) else {},
                        permissoes=dados.get("permissoes") if isinstance(dados.get("permissoes"), dict) else {},
                    )
                )
            inseridos += 1
    ...
    for caminho in sorted(caminhos):
        ...
        existente = session.scalar(
            select(PerfilSalvo).where(
                PerfilSalvo.cliente_usuario == cliente,
                PerfilSalvo.instagram_pk == instagram_pk,
            )
        )
        if existente:
            continue
        ...
        if not dry_run:
            session.add(
                PerfilSalvo(
                    ...
                    username=str(perfil.get("username") or "").strip() or None,
                    perfil=perfil,
                    caminho_historico_salvo=str(dados.get("caminho_historico_salvo") or "").strip() or None,
                )
            )
        inseridos += 1
    ...
    for caminho in sorted(caminhos):
        ...
        existente = session.scalar(
            select(Monitoramento).where(
                Monitoramento.cliente_usuario == cliente,
                Monitoramento.instagram_pk == instagram_pk,
            )
        )
        if existente:
            continue
        ...
        if not dry_run:
            session.add(
                Monitoramento(
                    cliente_usuario=cliente,
                    instagram_pk=instagram_pk,
                    username=str(dados.get("username") or "").strip() or None,
                    monitorando=bool(dados.get("monitorando", False)),
                    sleep=int(dados.get("sleep", 10) or 10),
                    dados=dados,
                    atualizado_em=normalizar_datetime(dados.get("atualizado")),
                )
            )
        inseridos += 1
...
def chave_historico(cliente: str, item: dict) -> tuple[str, str, str, str]:
    ...
    return (
        cliente,
        str(perfil.get("pk") or ""),
        str(item.get("timestamp_capture") or ""),
...
def migrar_historicos(session: Session, dry_run: bool) -> int:
    ...
    for registro in session.scalars(select(HistoricoPerfil)).all():
        ...
        existentes.add(
            (
                registro.cliente_usuario,
                registro.instagram_pk,
                registro.timestamp_capture.isoformat() if registro.timestamp_capture else "",
                str(dados.get("hash") or ""),
            )
        )
...
def chave_feed(item: dict) -> tuple[str, str, str, str, str]:
    return (
        str(item.get("cliente_usuario") or ""),
        str(item.get("pk") or ""),
        str(item.get("timestamp_capture") or ""),
        str(item.get("username") or ""),
        str(item.get("hash_atividade") or item.get("mensagem") or ""),
    )
        ...
        for item in ler_lista(caminho):
            ...
            if not dry_run:
                session.add(
                    FeedItem(
                        cliente_usuario=cliente,
                        timestamp_capture=normalizar_datetime(item.get("timestamp_capture")),
                        movimento=item.get("movimento") if isinstance(item.get("movimento"), bool) else None,
                        item=item,
                    )
                )
            existentes.add(chave)
...
def executar(dry_run: bool = False) -> dict[str, int]:
    ...
    etapas = (
        ...
        ("sessoes", migrar_sessoes),
        ("perfis_salvos", migrar_perfis_salvos),
        ("monitoramentos", migrar_monitoramentos),
        ("historico_perfis", migrar_historicos),
        ("feed_itens", migrar_feed),
    )

...
def executar() -> dict[str, int]:
    ...
    for item in atividades_validas:
        ...
        # A migração é idempotente por visitante + instante. O tipo legado
        # não participa da chave porque o runtime registra a visita como
        # "visita" independentemente do formato antigo.
        with Session(get_engine()) as session:
            existente = session.scalar(
                select(AtividadeVisitante.id).where(
                    AtividadeVisitante.visitante_id == visitante_id,
                    AtividadeVisitante.timestamp == timestamp,
                )
            )
    ...
    for item in visitantes:
        ...
        with Session(get_engine()) as session:
            existente = session.scalar(
                select(Visitante.id).where(Visitante.visitante_id == visitante_id)
            )
        if existente is not None:
            continue
    ...
    return {
        ...
        "atividades_ja_migradas": ignoradas,

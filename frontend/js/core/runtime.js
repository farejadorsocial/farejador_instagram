...
function nav(){const auth=state.session?.autenticado,items=auth?['dashboard|Painel','analyze|Analisar','saved|Usuários salvos','feed|Feed','explore|Explorar','ranking|Ranking','compare|Comparar']:['dashboard|Início','explore|Explorar','ranking|Ranking','feed|Feed','compare|Comparar'];const navEl=$('#main-nav');if(navEl)navEl.innerHTML=items.map(x=>{let[a,b]=x.split('|');return `<button class="${state.route===a?'active':''}" data-route="${a}">${b}</button>`}).join('');const top=$('.top-actions');if(top&&auth){top.innerHTML='<button id="theme-toggle" class="icon-btn" title="Tema">☾</button><button id="creditos-btn" class="credit-balance" title="Comprar créditos">💳 <span id="creditos-saldo">…</span></button><button id="auth-btn" class="outline-btn">Sair</button>'}const theme=$('#theme-toggle');if(theme)theme.onclick=()=>{state.theme=state.theme==='dark'?'light':'dark';applyTheme()};const authBtn=$('#auth-btn');if(authBtn)authBtn.textContent=auth?`Sair · ${state.session.cliente_usuario}`:'Entrar';document.body.classList.remove('mobile-nav-open');const m=$('#mobile-menu-toggle');if(m)m.setAttribute('aria-expanded','false')}
async function boot(){
  detectRoute();

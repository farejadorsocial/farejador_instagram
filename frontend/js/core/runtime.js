const state={session:null,route:'dashboard',profiles:[],analysis:null,analysisError:null,summary:null,summaryPk:null,historyField:null,publicProfile:null,compare:null,explore:null,ranking:null,exploreRank:'activity',exploreCategory:'',rankingCategory:'',publicUsername:null,feedFilter:'todos',feedTimer:null,feedAbortController:null,pageConfig:null,displayLimits:null,theme:localStorage.getItem('farejador-theme')||'dark'};
const $=s=>document.querySelector(s);
const API_CACHE_TTL={'/api/session':5000,'/api/config/atualizacao-paginas':30000,'/api/creditos':5000};
...
function go(route,replace=false){cancelarAtualizacoes();state.route=route;const path=route==='dashboard'?'/':route==='public-profile'?`/perfil/${encodeURIComponent(state.publicProfile?.perfil?.username||state.publicUsername||'')}`:route==='compare'?'/comparar':route==='explore'?'/explorar':route==='ranking'?'/ranking':'/';if(replace)history.replaceState({},'',path);else history.pushState({},'',path);render()}
function detectRoute(){const m=location.pathname.match(/^\/perfil\/([^/]+)$/);if(m){state.route='public-profile';state.publicUsername=decodeURIComponent(m[1]);return}if(location.pathname==='/comparar'){state.route='compare';return}if(location.pathname==='/explorar'){state.route='explore';return}if(location.pathname==='/ranking'){state.route='ranking';return}state.route='dashboard'}
function nav(){const auth=state.session?.autenticado,items=auth?['dashboard|Painel','analyze|Analisar','saved|Usuários salvos','feed|Feed','explore|Explorar','ranking|Ranking','compare|Comparar']:['dashboard|Início','explore|Explorar','ranking|Ranking','feed|Feed','compare|Comparar'];const navEl=$('#main-nav');if(navEl)navEl.innerHTML=items.map(x=>{let[a,b]=x.split('|');return `<button class="${state.route===a?'active':''}" data-route="${a}">${b}</button>`}).join('');const top=$('.top-actions');if(top){top.innerHTML=auth?'<button id="theme-toggle" class="icon-btn" title="Tema">☾</button><button id="creditos-btn" class="credit-balance" title="Comprar créditos">💳 <span id="creditos-saldo">…</span></button><button id="auth-btn" class="outline-btn">Sair</button>':'<button id="theme-toggle" class="icon-btn" title="Tema">☾</button><button id="auth-btn" class="outline-btn">Entrar</button>'}const theme=$('#theme-toggle');if(theme)theme.onclick=()=>{state.theme=state.theme==='dark'?'light':'dark';applyTheme()};const authBtn=$('#auth-btn');if(authBtn)authBtn.textContent=auth?`Sair · ${state.session.cliente_usuario}`:'Entrar';document.body.classList.remove('mobile-nav-open');const m=$('#mobile-menu-toggle');if(m)m.setAttribute('aria-expanded','false')}
async function boot(){
  detectRoute();
  const [sessionResult,configResult]=await Promise.allSettled([api('/api/session'),api('/api/config/atualizacao-paginas')]);
  if(sessionResult.status==='fulfilled')state.session=sessionResult.value;else{state.session={autenticado:false,cliente_usuario:null,modo_publico:true,publico_cliente_usuario:'admin',versao:null};console.error('Falha ao iniciar sessão:',sessionResult.reason)}
  if(configResult.status==='fulfilled')state.pageConfig=configResult.value;else{state.pageConfig={feed:{intervalo_segundos:2,ativo:true},paginas:{intervalo_segundos:10}};console.error('Falha ao carregar configuração:',configResult.reason)}
  applyTheme();
  await render();
}

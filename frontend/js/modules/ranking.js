(function(){
  ...
  const categoryIcon=name=>({
    Música:'🎵',Celebridade:'⭐',Ator:'🎭',Atriz:'🎭',Influenciador:'🎤',Esporte:'🏆',Futebol:'⚽',
    Humor:'😂',Jornalismo:'📰',Streamer:'🎮','Empresa/Marca':'🏢','Criador de conteúdo':'📱',Outro:'📌'
  }[name]||'📌');

  const metricValue=(x,tipo)=>{
  ...
  const metricLabel=(x,tipo)=>{
    const v=metricValue(x,tipo);
  ...
  const card=(x,rank,tipo)=>{
    const p=x.perfil||{};
    return `<button class="card category-ranking-card" data-public-profile="${esc(p.username||'')}">
      <span class="category-ranking-position">${rank}</span>
      ...
      <div class="public-profile-info">
        ...
        <div class="profile-stats"><span>👥 ${fmtNumber(p.seguidores)}</span><span>📈 ${x.crescimento>0?'+':''}${fmtNumber(x.crescimento)}</span></div>
        <small class="category-ranking-description">${esc(description(x,tipo))}</small>
      </div>
      <div class="category-ranking-score">${esc(metricLabel(x,tipo))}</div>
    </button>`;
  };

  const section=(title,subtitle,items,tipo)=>`<section class="category-ranking-section">
    <div class="section-title category-ranking-title"><div><b>${title}</b><small>${subtitle}</small></div><span class="ranking-label">TOP ${(items||[]).length}</span></div>
    <div class="category-ranking-list">${(items||[]).map((x,i)=>card(x,i+1,tipo)).join('')||'<div class="card empty">Ainda não há dados suficientes nesta categoria.</div>'}</div>
  </section>`;
  ...
  window.rankingView=async function(){
    ...
    return layout('🏆 RANKING','Rankings organizados por categoria',`
      <div class="card ranking-category-header">
        <div class="ranking-category-copy">
          <div class="eyebrow">RANKING POR CATEGORIA</div>
          <h2>${esc(scope)}</h2>
          <p class="muted">${state.session.autenticado?'Veja exclusivamente os perfis salvos na sua conta.':'Veja exclusivamente os perfis disponíveis na área pública.'}</p>
        </div>
        <label for="ranking-page-category">Categoria<select id="ranking-page-category" class="input">${categoryOptions}</select></label>
      </div>
      <div class="stats ranking-page-stats">${stat('👤 Perfis',d.total_perfis)}${stat('📸 Capturas',d.total_capturas)}${stat('📈 Eventos',d.total_eventos)}</div>
      <div class="ranking-page-grid">${sections}</div>
      <div class="notice public-disclaimer">${state.session.autenticado?'Nada desta tela usa dados de outros clientes.':'Os rankings públicos podem mudar quando novas capturas forem realizadas.'}</div>`);
  };

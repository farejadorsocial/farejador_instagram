function bindImages(){document.querySelectorAll('.profile-image').forEach(img=>{
  ...
  if(!wrap){wrap=document.createElement('div');img.parentNode.insertBefore(wrap,img);wrap.className='avatar-wrap';wrap.appendChild(img);const fallback=document.createElement('div');fallback.className='avatar-fallback';fallback.textContent='◉';fallback.setAttribute('aria-hidden','true');wrap.appendChild(fallback)}
...
function resetViewState(){cancelarAtualizacoes();state.analysis=null;state.analysisError=null;state.summary=null;state.summaryPk=null;state.historyField=null;state.profiles=[];state.explore=null;state.compare=null;state.exploreRank='activity';state.feedFilter='todos'}
function startFeedAutoRefresh(){
  ...
  const schedule=()=>{
    if(state.route!=='feed'||document.hidden)return;
    state.feedTimer=setTimeout(async()=>{
      state.feedTimer=null;
      if(state.route!=='feed'||document.hidden){schedule();return}
      const controller=new AbortController();
      state.feedAbortController=controller;
      try{
        const f=await api('/api/feed',{signal:controller.signal,timeoutMs:10000});
        if(controller.signal.aborted||state.route!=='feed')return;
        ...
        if(e.name!=='AbortError'){
          ...
          if(last)last.textContent=e.status===401?'Sessão expirada. Entre novamente.':'Aguardando atualização…';
        }
      }finally{
        if(state.feedAbortController===controller)state.feedAbortController=null;
        if(state.route==='feed'&&!document.hidden)schedule();
      }
    },segundos*1000);
  };
  schedule();
...
function bind(){
  ...
  if(btn)btn.onclick=async()=>{
    try{
      ...
      const result=await api('/api/profile/analyze',{method:'POST',body:JSON.stringify({username}),timeoutMs:30000});
      ...
      render();
    }catch(e){state.analysis=null;state.analysisError=e.name==='AbortError'?'A consulta demorou mais que o esperado.':e.message||'Usuário não encontrado.';render()}
  ...
  if(save)save.onclick=async()=>{try{await api('/api/profile/save',{method:'POST',body:JSON.stringify({dados:state.analysis})});state.profiles=await api('/api/profiles');toast('Usuário salvo na sua conta.');render()}catch(e){toast(e.message)}};
  ...
  document.querySelectorAll('[data-monitor]').forEach(b=>b.onclick=async()=>{try{await api(`/api/profiles/${encodeURIComponent(b.dataset.monitor)}/monitor`,{method:'POST',body:JSON.stringify({monitorando:b.dataset.enabled==='true'})});toast('Monitoramento atualizado.');render()}catch(e){toast(e.message)}});
  document.querySelectorAll('[data-remove]').forEach(b=>b.onclick=async()=>{if(!confirm(`Remover @${b.dataset.remove}?`))return;try{await api(`/api/profiles/${encodeURIComponent(b.dataset.remove)}`,{method:'DELETE'});state.profiles=await api('/api/profiles');toast('Usuário removido.');render()}catch(e){toast(e.message)}});

  const search=$('#profile-search');if(search)search.oninput=()=>{const q=search.value.toLowerCase();document.querySelectorAll('.profile-card').forEach(e=>e.style.display=e.dataset.username.toLowerCase().includes(q)?'grid':'none')};
  ...
  document.querySelectorAll('[data-compare-from]').forEach(b=>b.onclick=()=>{state.compare={a:b.dataset.compareFrom||'',b:''};go('compare')});
  document.querySelectorAll('[data-login]').forEach(b=>b.onclick=openAuth);
  document.querySelectorAll('[data-feed-filter]').forEach(b=>b.onclick=()=>{const filter=b.dataset.feedFilter;state.feedFilter=filter;document.querySelectorAll('#feed-list .feed-row').forEach(row=>{const movimento=row.dataset.movimento==='true';row.style.display=filter==='todos'?'flex':filter==='movimento'?(movimento?'flex':'none'):(movimento?'none':'flex')});document.querySelectorAll('[data-feed-filter]').forEach(x=>x.classList.toggle('active-filter',x.dataset.feedFilter===filter))});

  const ranking=$('#ranking-select');if(ranking)ranking.onchange=()=>{state.exploreRank=ranking.value;render()};
  document.querySelectorAll('[data-compare-toggle]').forEach(button=>button.onclick=e=>{e.preventDefault();const side=button.dataset.compareToggle,menu=$(`#compare-menu-${side}`);if(!menu)return;const willOpen=!menu.classList.contains('open');document.querySelectorAll('.compare-select-menu.open').forEach(x=>x.classList.remove('open'));document.querySelectorAll('[data-compare-toggle]').forEach(x=>x.setAttribute('aria-expanded','false'));if(willOpen){menu.classList.add('open');button.setAttribute('aria-expanded','true')}});
  document.querySelectorAll('[data-compare-option]').forEach(option=>option.onclick=()=>{const side=option.dataset.compareOption,user=option.dataset.compareUser,current=state.compare||{};state.compare={...current,[side]:user,data:null};render()});
  const compareSwap=$('#compare-swap');if(compareSwap)compareSwap.onclick=()=>{const current=state.compare||{},a=current.a,b=current.b;state.compare={...current,a:b,b:a,data:null};render()};
  ...
  if(compareBtn)compareBtn.onclick=async()=>{
    ...
    try{
      ...
      state.compare={a,b,data:await api(`${endpoint}?username_a=${encodeURIComponent(a)}&username_b=${encodeURIComponent(b)}`,{timeoutMs:30000})};
      history.replaceState({},'',`/comparar?a=${encodeURIComponent(a)}&b=${encodeURIComponent(b)}`);render();
    }catch(e){toast(e.message)}finally{compareBtn.disabled=false;compareBtn.textContent='⚡ COMPARAR AGORA'}
  };
...
document.addEventListener('farejador:session-expired',()=>{cancelarAtualizacoes();resetViewState();toast('Sua sessão expirou. Entre novamente para continuar.');render()});
document.addEventListener('visibilitychange',()=>{if(state.route==='feed'){if(document.hidden)cancelarAtualizacoes();else startFeedAutoRefresh()}});

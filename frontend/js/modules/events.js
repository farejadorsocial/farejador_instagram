...
function startFeedAutoRefresh(){
  ...
  const schedule=()=>{
    ...
    state.feedTimer=setTimeout(async()=>{
      ...
      const controller=new AbortController();state.feedAbortController=controller;
      try{
        ...
        document.querySelectorAll('#feed-list .feed-row').forEach(row=>{const movimento=row.dataset.movimento==='true';row.style.display=state.feedFilter==='todos'?'flex':state.feedFilter==='movimento'?(movimento?'flex':'none'):(movimento?'none':'flex')});
        const last=$('#feed-last-refresh');if(last)last.textContent=`Última leitura: ${fmtDate(new Date())}`;bindImages();
      }catch(e){if(e.name!=='AbortError'){const last=$('#feed-last-refresh');if(last)last.textContent=e.status===401?'Sessão expirada. Entre novamente.':'Aguardando atualização…'}}
      finally{if(state.feedAbortController===controller)state.feedAbortController=null;if(state.route==='feed'&&!document.hidden)schedule()}
    },segundos*1000);
  };
  schedule();
}
...
function bind(){
  ...
  const ps=$('#public-search-input'),pb=$('#public-search-btn');if(pb)pb.onclick=()=>publicSearch();if(ps){let timer;ps.oninput=()=>{clearTimeout(timer);timer=setTimeout(()=>publicSearch(ps.value),250)};ps.onkeydown=e=>{if(e.key==='Enter'){e.preventDefault();publicSearch(ps.value)}}}
  bindPublicCards();

(function(){
  ...
  window.exploreView=async function(){
    ...
    const rankingSubtitle=categoria?selected[2]+' · '+categoria:selected[2];
    const publicText=state.session.autenticado?'Os cálculos usam exclusivamente os dados dos usuários salvos por você.':'Os rankings usam somente o histórico público já registrado.';
    const disclaimer=state.session.autenticado?'Nada desta tela usa dados públicos de outros clientes.':'Os rankings podem mudar quando novas capturas forem realizadas.';
    const options=ranks.map(r=>'<option value="'+esc(r[0])+'" '+(r[0]===state.exploreRank?'selected':'')+'>'+r[1]+'</option>').join('');
    const html=publicSearchBox()
      +'<div class="stats explore-stats">'+stat('👤 Perfis',d.total_perfis)+stat('📸 Capturas',d.total_capturas)+stat('📈 Eventos',d.total_eventos)+'</div>'
      +'<div class="explore-intro card"><div><div class="eyebrow">RANKING DO MOMENTO</div><b>Veja quem mais se destaca</b><p class="muted">'+publicText+'</p></div><button class="primary-btn" data-route="compare">⚖ Comparar perfis</button></div>'
      ...
      +'<div class="ranking-selector card"><div class="ranking-filter-grid"><label for="ranking-select">Ranking exibido<select id="ranking-select" class="input">'+options+'</select></label><label for="ranking-category-select">Categoria<select id="ranking-category-select" class="input">'+categoryOptions+'</select></label></div>'
      +'<div class="ranking-filter-status"><span>'+categoryIcon(categoria||'Outro')+'</span><div><b>'+esc(scopeLabel)+'</b><small>'+(categoria?'Ranking filtrado por categoria':'Ranking geral com todos os perfis')+'</small></div></div>'
      +'<div class="ranking-selected-info"><b>'+selected[1]+'</b><span>'+esc(rankingSubtitle)+'</span></div></div>'
      +exploreSection(selected[1],rankingSubtitle,items,state.exploreRank)
      +'<div class="notice public-disclaimer">'+disclaimer+'</div>';
    return layout('◈ EXPLORAR',state.session.autenticado?'Explore somente os perfis salvos na sua conta':'Descubra perfis, crescimento, atividade e rankings públicos',html);
  };

  window.__farejadorEnhancedExplore=true;

  document.addEventListener('change',event=>{
    if(event.target?.id!=='ranking-category-select')return;
    state.exploreCategory=String(event.target.value||'').trim();
    render();
  },true);
})();
